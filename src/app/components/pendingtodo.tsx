...
export default function PendingTodos({
  ...
  return (
    <div className="mb-8">
      ...
      {todos.length === 0 ? (
        <div className="text-center py-6 text-gray-500">No active tasks. Add one above!</div>
      ) : (
                ...
                {todos.map((todo, index) => (
                  <Draggable key={todo.id} draggableId={todo.id} index={index}>
                    {(provided, snapshot) => (
                      ...
                      >
                        <div
                          className={`flex items-center p-4 bg-white rounded-lg border-l-4 ${
                        ...
                        >
                          ...
                          <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            {editingId !== todo.id && (
                              <>
                                <button onClick={() => onStartEdit(todo.id, todo.task)} className="p-1 text-blue-500 hover:text-blue-700" title="Edit">
                                  <FiEdit2 size={18} />
                                </button>
                                <button onClick={() => onComplete(todo.id)} className="p-1 text-green-500 hover:text-green-700" title="Complete">
                                  <FiCheck size={18} />
                                </button>
                              </>
                            )}
                            <button onClick={() => onDelete(todo.id)} className="p-1 text-red-500 hover:text-red-700" title="Delete">
                              <FiTrash2 size={18} />
                            </button>
                          </div>
                        </div>
